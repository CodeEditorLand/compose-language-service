...
import { type TelemetryEvent, initEvent } from "../../../client/TelemetryEvent";
...
export class TelemetryAggregator implements Disposable {
	...
	public constructor(
		...
		private readonly interval: number = FlushIntervalMilliseconds,
	) {
	...
	public logEvent(event: TelemetryEvent): void {
		if (event.suppressAll) {
			...
			event.properties.result === "Succeeded" &&
			event.suppressIfSuccessful
		) {
	...
	private getAggregatedEvents(): TelemetryEvent[] {
		...
		for (const events of eventGroups.values()) {
			...
			events.forEach((evt) =>
				Object.assign(aggregatedEvent.properties, evt.properties),
			);

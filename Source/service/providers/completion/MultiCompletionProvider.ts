 ...
 */
export class MultiCompletionProvider extends ProviderBase<
	CompletionParams & ExtendedParams,
...
> {
	...
	public constructor(
		private readonly basicCompletions: boolean,
		private readonly advancedCompletions: boolean,
	) {
		super();
	...
	public override async on(
		params: CompletionParams & ExtendedPositionParams,
		...
		for (const collection of this.completionCollections) {
			...
			const subresults =
				collection.getActiveCompletionItems(extendedParams);
			...
			ctx.telemetry.properties.respondingCollections =
				respondingCollections.sort().join(",");
		}

...
export class CompletionCollection extends Array<ExtendedCompletionItem> {
	...
	public getActiveCompletionItems(
		params: ExtendedCompletionParams,
	): CompletionItem[] | undefined {
		if (
			this.locationRequirements.logicalPaths !== undefined &&
			!this.locationRequirements.logicalPaths.some((p) =>
				p.test(params.positionInfo.path ?? ""),
			)
		) {
		...
		if (
			this.locationRequirements.indentationDepth !== undefined &&
			this.locationRequirements.indentationDepth !==
				params.positionInfo.indentDepth
		) {

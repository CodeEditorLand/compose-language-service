...
export class KeyHoverProvider extends ProviderBase<
	HoverParams & ExtendedParams,
	Hover | undefined,
	never,
	never
> {
	public async on(
		params: HoverParams & ExtendedParams,
			...
			if (lineKeyName === pathKeyName) {
				...
				return {
					contents: {
						kind: preferMarkdown
							? MarkupKind.Markdown
							: MarkupKind.PlainText, // If Markdown is preferred, even plaintext will be treated as Markdown--it renders better, has line wrapping, etc.
						value:
							(preferMarkdown && keyInfo.markdownContents) ||
							keyInfo.plaintextContents,
					},
					range: Range.create(
						Position.create(params.position.line, keyIndex),
						Position.create(
							params.position.line,
							keyIndex + lineKeyName.length,
						),
					),
				};
			}

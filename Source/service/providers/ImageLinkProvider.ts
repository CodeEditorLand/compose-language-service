...
import { Scalar, isMap, isScalar } from "yaml";
...
export class ImageLinkProvider extends ProviderBase<
	DocumentLinkParams & ExtendedParams,
	DocumentLink[] | undefined,
	never,
	never
> {
	public on(
		params: DocumentLinkParams & ExtendedParams,
			...
			for (const service of serviceMap.items) {
				...
				if (isMap(service.value)) {
					...
					if (
						...
						const quoteOffset =
							image.type === Scalar.QUOTE_SINGLE ||
							image.type === Scalar.QUOTE_DOUBLE
								? 1
								: 0; // Offset if the scalar is quoted
						const link = ImageLinkProvider.getLinkForImage(
							image.value,
							imageTypes,
						);
		...
		ctx.telemetry.properties.imageTypes = Array.from(imageTypes.values())
			.sort()
			.join(",");
	...
	private static getLinkForImage(
		...
		if (
			(match = dockerHubImageRegex.exec(image)) &&
			...
			(match = dockerHubNamespacedImageRegex.exec(image)) &&
			...
			(match = mcrImageRegex.exec(image)) &&
			...
			return {
				uri: `https://hub.docker.com/_/microsoft-${namespace.replace(
					"/",
					"-",
				)}-${imageName}`,
				start: match.index,

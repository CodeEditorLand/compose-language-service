...
export class DiagnosticProvider extends ProviderBase<
	TextDocumentChangeEvent<ComposeDocument> & ExtendedParams,
	void,
	never,
	never
> {
	...
	public on(
		params: TextDocumentChangeEvent<ComposeDocument> & ExtendedParams,
	): void {
		...
		debounce(
			...
			{ uri: params.document.textDocument.uri, callId: "diagnostics" },
			() => {
				const diagnostics: Diagnostic[] = [];

				for (const error of [
					...params.document.yamlDocument.value.errors,
					...params.document.yamlDocument.value.warnings,
				]) {
					diagnostics.push(
						Diagnostic.create(
							yamlRangeToLspRange(

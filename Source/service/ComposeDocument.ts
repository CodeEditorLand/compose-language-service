...
export class ComposeDocument {
	...
	public lineAt(line: Position | number): string {
		...
		if (startOfLine.line > this.textDocument.lineCount) {
			throw new Error(
				`Requested line ${startOfLine.line} is out of bounds.`,
			);
		}
	...
	public async getSettings(): Promise<DocumentSettings> {
		...
		if (!this.documentSettings) {
			...
			if (
				ctx.clientCapabilities?.experimental?.documentSettings?.request
			) {
				const result = await ctx.connection.sendRequest<
					DocumentSettingsParams,
	...
	public async getPositionInfo(
		...
		for (
			...
			if ((result = ItemValueRegex.exec(currentLine))) {
				...
				if (
					indentDepth < currentIndentDepth ||
					(indentDepth === currentIndentDepth &&
						fullPathParts[0] === Item)
				) {
	...
	private buildYamlDocument(): YamlDocument<YamlNode> {
		const yamlDocument = parseDocument(this.textDocument.getText(), {
			merge: true,
			prettyErrors: true,
			customTags: ComposeCustomTags,
		});
	...
	private guessDocumentSettings(): DocumentSettings {
		...
		const eol = /\r/gi.test(documentText) ? CRLF : LF;
	...
	private getFirstLinePositionInfo(
		...
		if ((result = LineWithCommentRegex.exec(currentLine))) {
			const commentSepPosition = currentLine.indexOf(
				result.groups!["commentSep"],
			);
...
export const KeyValueRegex =
	/^(?<indent> *)(?<key>(?<keyName>[.\w-]+)(?<keyInd>(?<keySep>:) *))(?<value>.*)$/im;

// A regex for matching an item/value line, i.e. `- value`
const ItemValueRegex =
	/^(?<indent> *)(?<itemInd>(?<itemSep>-) *)(?<value>.*)$/im;

// A regex for matching an item/key/value line, i.e. `- key: value`. This will be the top line of a flow map.
const ItemKeyValueRegex =
	/^(?<indent> *)(?<itemInd>(?<itemSep>-) *)(?<key>(?<keyName>[.\w-]+)(?<keyInd>(?<keySep>:) *))(?<value>.*)$/im;
